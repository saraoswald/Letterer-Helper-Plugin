...
Inline.Properties=class Properties extends Style.Properties{
	rFonts(x){
		 x && x.ascii && this.style && (this.style.fontFamily=x.ascii)
	}
	b(x){
		this.style && (this.style.fontWeight=700)
	}
	sz(x){
		this.style && (this.style.fontSize = x + 'px')
	}
	color(x){
		this.style && (this.style.color=x)
	}
	i(x){
	...
	bdr(x){
		this.style && (this.style.border=this._border(x))
	}
	lang(x){
